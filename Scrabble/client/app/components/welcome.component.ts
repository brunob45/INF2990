...
export class WelcomeComponent {
    ...
    login(username: string, password: string): void
    {
        ...
        this.http.post(SERVER_LINK + "login", body, {headers: headers}).subscribe(data =>
        {
            ...
            {
                window.alert("La connection a echoué. Soit le nom d'utilisateur ou le mot de passe " +
                             "est erronné, soit l'utilisateur est deja connecté.");
            }
        });
    }
    ...
    private setUser(username: string, password: string): void
    {
        ...
        Sockets.connect();
        Sockets.userConnect({username: username, password: password});
    }
}
